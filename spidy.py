    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            ...
            if user_to_add not in allowed_user_ids:
                ...
                response = "Are Bas Kar 1 Bande ko Kitni bar Dale Ga Noob."
        else:
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.☠️🕸️\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: VIP-UDP-BGMI BY:- @SPIDY_OWNEROP"
    bot.reply_to(message, response)
...
🚀 Official Channel :- @SPIDY_OWNEROP
'''
...
def welcome_start(message):
    ...
    response = f'''👋🏻WECOME DDoS BOT @SPIDY_OWNEROP
